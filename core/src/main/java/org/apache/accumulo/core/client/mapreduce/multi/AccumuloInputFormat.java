...
public class AccumuloInputFormat extends InputFormatBase<TableKey,Value> {
  @Override
  public RecordReader<TableKey,Value> createRecordReader(final InputSplit split, final TaskAttemptContext context) throws IOException, InterruptedException {
    log.setLevel(getLogLevel(context.getConfiguration()));
    return new RecordReaderBase<TableKey,Value>() {
      private Text table;
      @Override
      public void initialize(InputSplit inSplit, TaskAttemptContext attempt)
          throws IOException {
        super.initialize(inSplit, attempt);
        RangeInputSplit split = (RangeInputSplit) inSplit;
        table = new Text(split.getTableName());
      }
      ...
      public boolean nextKeyValue() throws IOException, InterruptedException {
        if (scannerIterator.hasNext()) {
          ...
          currentK = new TableKey(table, currentKey);
